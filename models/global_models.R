...
start_forecasting <- function(dataset, lag, forecast_horizon){
  ...
  fit_model(embedded_series, lag, final_lags, forecast_horizon)
}


# Fit and forecast from a global model
fit_model <- function(fitting_data, lag, final_lags, forecast_horizon) {
  # Create the formula
  formula <- "y ~ "
  ...
  # Do forecasting
  forec_recursive(lag, model, final_lags, forecast_horizon)
}

  
# Recursive forecasting of the series until a given horizon
forec_recursive <- function(lag, model, final_lags, forecast_horizon) {
